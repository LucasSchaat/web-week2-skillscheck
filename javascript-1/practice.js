...
const doubled = myNumbers.map(item => {return item*2})

//////////////////PROBLEM 15////////////////////

// Now lets use .filter to only get the nubmer(s) greater than 100. Call the new array 'filtered'

const filtered = myNumbers.filter(item => {if (item > 100) {return item}})

//////////////////PROBLEM 16////////////////////

// Next up, reduce to get the total of the array. call the new array 'total'

const total = myNumbers.reduce((acc,current) => {return acc += current},0)
...
const forTheLoveOfGeorge = notGeorge.map(item => {return item = 'George'})
...
const totallyAwesome = people.reduce((acc,curr) => {return acc = acc + curr.awesomeLevel},0)
