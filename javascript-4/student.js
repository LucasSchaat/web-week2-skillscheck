...
const faveColors = ['red','blue','grey']
...
let me = {
    firstName: 'Lucas',
    superHeroName: 'ElGuapote',
    homeTown: 'Rexburg',
    superPowers: ['master vision','master negotiator','provider of world peace'],
...
function setColor (arr) {
    let backgroundArr = arr.splice(0,3)
    for (i=0; i<backgroundArr.length; i++) {
        if (backgroundArr[i] === 'blue') {
            backgroundArr[i] = '#4D4DFF'
        }
    }
    background (backgroundArr[0],backgroundArr[1],backgroundArr[2])
}
...
function redactInfo (obj) {
    Object.keys(obj).forEach(item => {obj[item] = 'redacted'})
    redacted()
    return obj
}